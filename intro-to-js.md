...
Cuando aprendemos un nuevo lenguaje de programación, es muy importante coger los ejemplos que se nos dan, probarlos, modificarlos y tratar así de ampliar nuestro entendimiento del lenguaje. Para hacer esto, por supuesto, necesitamos un intérprete.

La manera más fácil de probar unas líneas de JS es abrir las herramientas de desarrollo de tu navegador y seleccionar la tab de la Consola. Ahí podemos incluir código y ver el resultado de lo que escribimos. Muchas veces vienen en forma de paneles estas consolas, pero suele ser posible separarlas en ventanas independientes, lo cual facilita el probar código ;D
...
Escribir código en la consola del navegador o en la terminal de una sesión de Node puede resultar engorroso, y profesionalmente ineficiente. Es mejor que usemos un editor de código como VSCode, WebStorm, etc. Desde ahí sí podemos escribir código que después copiemos y peguemos en las consolas de los navegadores o en una sesión de Node. O lo que es mejor todavía, podemos guardar el código en un archivo JS (su extensión debe ser .js) y después ejecutarlo con Node:

`$ node archivo.js`

Si usamos Node de esta manera, hay que tener en cuenta que Node no mostrará el resultado por pantalla como en las sesiones, hay que indicarlo de una manera programática. Podemos usar la función `console.log()` para mostrar texto y otros valores de JS en la terminal o en la consola del navegador.
...
x = "js mola"; // Strings de texto con comillas simples
...
Los otros 2 tipos importantes que JS puede manipular son los objetos y los arrays.
...
3 / 2; // => 1.5: división
points[1].x - points[0].x; // 1 => operadores más complicados también funcionan
"3" + "2"; // => "32": + suma números y concatena arrays

// JS define algunas abreviaciones para operadores aritméticos comunes
let count = 0;
count++; // Incrementa la variable
count--; // Decrementa la variable
count += 2; // Suma 2: parecido a hacer count = count + 2;
count *= 3; // Multiplica por 3: parecido a hacer count = count * 3;
...
"dos" > "tres"; // => true: " dos "es alfabéticamente mayor que " tres "
...
Una función es un bloque con nombre y parametrizado de código JS que se define una sola vez, y puede ser reutilizado invocándolo tantas veces como sea necesario.

```js
// Las functiones son bloques parametrizados de JS que pueden ser invocadas
function plus1(x) {
  // Define una función llamada 'plus1' con el parámetro 'x'
  return x + 1; // Devuelve el valor con una adición de 1 al valor pasado a la función
...
square(plus1(y)); // 16: invoca dos funciones en una sola expresión
```

En ES6 y posteriores, existe un shortcut para la sintaxis de definición de funciones, llamado fat arrow functions. La sintaxis es más concisa y usa `=>` para separar una lista de argumentos del cuerpo de la función. Las arrow functions son muy usadas cuando queremos pasar una función anónima como argumento de otra función:
...
Cuando usamos funciones con objetos, obtenemos lo que se conocen como métodos:

```js
// Cuando a un objeto le asignamos como propiedad una función a estas las llamamos métodos.
// Todos los objetos JS (incluyendo arrays) tienen métodos

let a = [];
a.push(1, 2, 3); // el método push() añade elementos al array
a.reverse(); // el método reverse, invierte el array

// Podemos definir nuestros propios métodos.
// La palabra 'this' se refiere al objeto donde el propio método está definido

points.dist = function () {
  // definimos un método que calcula la distancia entre puntos
  ...
  */
  ...
  return Math.sqrt(a * a + b * b); // Math.sqrt() calcula la raiz cuadrada
};
...
JS también soporta el estilo de programación orientada a objetos, pero es un poco diferente a la programación orientada a objetos de otros lenguajes debido a su nacimiento como lenguaje que implementa la herencia de prototipos. En ES6 se incluyeron las clases `class` como azúcar sintáctico sobre las funciones, para darle un estilo a la sintaxis más parecido al de otros lenguajes.

```js
class Point {
  // Por convención el nombre de las clases debe ser capitalizado
  constructor(x, y) {
    // La función de construcción inicializa nuevas instancias
    this.x = x; // guardamos los argumentos de la función en propiedades del objeto
    this.y = y;
...
// Usamos la función del constructor de Point() con 'new' para crear objetos Point
const p = new Point(1, 1);
// Ahora podemos usar los métodos de Point sobre el objeto p
p.distance();
```
